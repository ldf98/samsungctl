...
def parseClientHello(clientHello, dataHash, aesKey, gUserId):
    ...
    finalBuffer = userId + gUserId.encode('utf-8') + pGx + binascii.unhexlify(keys.publicKey) + secret
    sha1 = hashlib.sha1()
...
def generateServerAcknowledge(SKPrime):
    ...
    SKPrimeHash = sha1.digest()
    return "0103000000000000000014"+bytes2str(binascii.hexlify(SKPrimeHash)).upper()+"0000000000"

def parseClientAcknowledge(clientAck, SKPrime):
    sha1 = hashlib.sha1()
    sha1.update(SKPrime+b"\x02")
    SKPrimeHash = sha1.digest()
    tmpClientAck = "0104000000000000000014"+bytes2str(binascii.hexlify(SKPrimeHash)).upper()+"0000000000"
