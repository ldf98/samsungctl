...
class SliderHandler(object):
    ...
    def __init__(self):
        ...
        self._tick_freq = None
        self._increment = None
        self._min_max_labels = False
        self._value_label = False
    ...
    @property
    def min_max_labels(self):
        return self._min_max_labels

    @min_max_labels.setter
    def min_max_labels(self, value):
        self.reset()
        self._min_max_labels = value
    ...
    def float_format(self):
        if self._float_format is None:
            if self.int_ctrl:
                self._float_format = '%d'
            else:
                inc_t = '%f' % self.increment
                max_t = '%f' % self.max_value
                min_t = '%f' % self.min_value
                inc_dec = len(inc_t.rstrip('0').split('.')[1])
                max_dec = len(max_t.rstrip('0').split('.')[1])
                min_dec = len(min_t.rstrip('0').split('.')[1])
                dec_pos = max([inc_dec, max_dec, min_dec])

                if not dec_pos:
                    dec_pos = 1

                self._float_format = '%.' + str(dec_pos) + 'f'
    ...
    def min_size(self):
        if self._min_size is None:
            ...
            self._min_size = dc.GetTextExtent(self.min_text)
            dc.Destroy()
            del dc
    ...
    def max_size(self):
        if self._max_size is None:
            dc = wx.MemoryDC()
            dc.SetFont(self.font)
            self._max_size = dc.GetTextExtent(self.max_text)
            dc.Destroy()
            del dc
    ...
    def value_size(self):
        if self._value_size is None:
            dc = wx.MemoryDC()
            dc.SetFont(self.font)
            self._value_size = dc.GetTextExtent(self.value_text)
            dc.Destroy()
            del dc
    ...
    @property
    def min_text(self):
        return self.float_format % self.min_value

    @property
    def max_text(self):
        return self.float_format % self.max_value

    @property
    def value_text(self):
        return self.float_format % self.value

    @property
    def slider_length(self):
        if self._slider_length is None:
            ...
            self._slider_len = slider_len
        return self._slider_len

    @property
    def tick_freq(self):
        return self._tick_freq

    @tick_freq.setter
    def tick_freq(self, value):
        self.reset()
        self._tick_freq = value
    ...
    def slider_steps(self):
        if self._slider_steps is None:

            if self.min_value > self.max_value:
                slider_steps = (self.min_value - self.max_value) / self.increment
            else:
                slider_steps = (self.max_value - self.min_value) / self.increment

            if slider_steps > 1000:
                slider_steps = 1000.0

            self._slider_steps = slider_steps / 2
    ...
    def selection_start(self, value):
    ...
    def set_selection(self, min_val, max_val):
        self.sel_start = min_val
        self.sel_end = max_val

    @property
    def min_max_position(self):
        if self._min_max_position is None:
            min_w, min_h = self.min_size
            max_w, max_h = self.max_size
            width, height = self.size
            ...
            if self.horizontal:
                ...
                max_x = width - max_w - 3
                if self.top:
                    min_y = (height / 2) - min_h
                    max_y = (height / 2) - max_h
                else:
                    min_y = (height / 2)
                    max_y = (height / 2)
            ...
            self._min_max_position = (min_x, min_y), (max_x, max_y)

        return self._min_max_position

    @property
    def slider_position(self):
        if self._slider_position is None:
            min_w, min_h = self.min_size
            max_w, max_h = self.max_size
            width, height = self.size
            ...
            for pos in [0, 1, 2]:
                ...
                if self.horizontal:
                    ...
                    y += pos
                    slider_lines.append([x, y, width - x2 - 4, y])
                else:
                    y = 5
                    y2 = 5
                    if self.min_max_labels:
                    ...
                    x += pos
                    slider_lines.append([x, y, x, height - y2 - 4])

            self._slider_position = slider_lines[:]
    ...
    def thumb(self):
        ...
        if self.inverse:
            min_val, max_val = self.max_value, self.min_value
        else:
            minVal, maxVal = self.MinValue, self.MaxValue

        thumbPos = int(
            (
                (value - minVal) / ((maxVal - minVal) / len(self.TickPosition))
            ) / self.Increment
        )
    ...
    def thumb(self, mouse_pos):
        ...
        new_value = (
            ((pos - min_r) * (max_val - min_val)) / (self.slider_length - min_r)
        ) + min_val
        ...
        if new_value is not None and max_val >= new_value >= min_val:
            self._value_size = None
            self._value_text = None
            self.value = value
    ...
    def value_position(self):
        if self._value_position is None:
            width, height = self.size
            val_w, val_h = self.value_size
            thumb_pos = self.GetThumb()

            if self.horizontal:
                val_x = thumb_pos - (val_w / 2)
                if self.top:
                    val_y = (height / 2) + 8
                else:
                    val_y = (height / 2) - val_h - 9
            else:
                val_y = thumb_pos - (val_h / 2)

                if self.left:
                    val_x = (width / 2) + 8
                else:
                    val_x = (width / 2) - val_w - 9

            self._value_position = (val_x, val_y)
...
# ----------------- PAINT
    ...
    def _Draw(self, pdc):
        ...
        dc.SetPen(wx.Pen(wx.Colour(*self.GetBackgroundColour()), 1))
        dc.Clear()
